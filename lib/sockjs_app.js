...
var Terminal = function(conn,user,timestamp){
	
	try{
		var term

			term = pty.spawn('./create-jail-and-chroot.sh', [timestamp], {
				name: 'xterm-color',
				 cols: 80,
				 rows: 30
			});
		...
		term.on('data', function(data) {				
			...
			if(startTime[conn.id]){
				...
				logger.log('debug',endTime[0] * 1000 + endTime[1]/1000000);
...
server.on('connection', function(conn) {
	...
	term = new Terminal(conn,username,timestamp);		
	...
	conn.on('close', function() {
		if(term){
			...
			exec("./remove-user.sh " + timestamp,function(error,stdout,stderr){
			});
		}
